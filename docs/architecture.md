...
- **Failed polls back off** (5s doubling to 60s): a failed `getUpdates` returns immediately rather than blocking for the long-poll timeout, so retrying at loop speed would hammer `api.telegram.org` for as long as the failure lasts. `get_updates()` therefore returns `None` on failure and `[]` on "no messages" — the two must stay distinguishable. Note the failure this mainly guards against is *structural*: Telegram allows exactly one `getUpdates` consumer per bot token, and the token is shared fleet-wide (see below), so every deployment beyond the one that wins the race gets a permanent HTTP 409 and never sees a `/start` handshake. Backoff bounds the damage; it does not fix the underlying single-consumer conflict.
