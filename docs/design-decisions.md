...
## 43. A run that traded and then fell short is `partial`, and `failed` now means nothing reached the exchange

**Decision**: `BotRunStatus` gains a sixth terminal value, `partial`, between `completed` and `failed`. A run is `partial` when **orders reached the exchange and a position is open**, but something after that fell short — the PB/SL stop never armed, or one leg of a multi-leg trade was rejected. `failed` is now reserved for the run where nothing went out and nothing is owed. The reason code continues to say *which* half is missing (`exit_arm_failed`, `exit_arm_skipped_existing_position`, `order_rejected`); the status says only whether the bot traded.

Every writer follows the same rule: the autonomous sweep (`bots/scheduler`), both approval paths (`bots/proposals`), Bot 1's unattended run (`bots/holdings_runner`), and `bots/exit_arming` when it settles a position whose stop never armed. In the UI a `partial` run is amber, not rose, and the card reads "2 placed · no stop" rather than "failed".

**Why**: on the 24-Sep-2026 SENSEX expiry the user approved a Bot 2 plan on Telegram, both legs were placed, and only the stop was declined — the group already held another open leg, so arming one there would have pooled P&L across both positions (#32's final, non-retryable case). The bot card and the Activity log both badged that run **FAILED**. That is the one reading that is definitely wrong: a live short was open, holding margin, with no automatic exit, and the message exists precisely to send the user to Portfolio to set PB/SL by hand. "Failed" says the opposite — nothing happened, nothing to do. It is the same mistake as reporting a failed arm as `order_rejected`, one level up: the status word had no way to say "it traded, and it needs you".

**The single-index gate was the other half of the bug.** `_tick_expiry_index_writer` took its early `if not ok:` exit whenever no result was clean — and an arm failure sets the result's `error`, so a lone index whose orders filled and whose stop was then refused had `ok` empty. That exit reported the run from `first.error` alone, never named what was sold, and skipped the Telegram warning below it. The gate now asks whether anything carried order ids, not whether anything was clean, so a filled-but-unprotected single index falls through to the normal reporting path.

**What does not change**: `partial` is a *terminal* status, so it counts as the bot having acted today exactly as `completed` does — no bot re-fires on top of a position it already opened. `bot_runs.status` has no CHECK constraint, so nothing migrates; rows written before this read back as they always did. And `exit_arm_pending` is still not a shortfall: a stop waiting on its fills is `completed`, because `exit_arming` arms it without anyone doing anything (#32).

---

## 44. The data volume has one threshold: past it the app warns on every page and backtests stop writing

**Decision** (2026-09-25). Settings → Diagnostics → **Storage** reads how full the data volume is, live, with `statvfs` on `DATA_PATH` from inside the container (`services/storage/usage.py`), lists what occupies it element by element with the dates each covers (`storage/elements.py`), and deletes any deletable element for a chosen date range (`storage/cleanup.py`). **One editable threshold** (default 85%, bounds 50–98, in `users.sqlite3`'s `storage_settings`) does three jobs at once: at or past it, every page shows an amber, non-dismissable banner under the licence banner; new backtests of every kind are refused; and a running backtest halts the parts that write to the volume.

**Only the data volume is measured.** The CloudFormation stack has two EBS volumes: the data volume bind-mounted at `backend/data`, and the root volume holding the OS and Docker images. Everything this screen can delete is on the first, so a banner driven by the second would ask for something the user cannot do. No AWS API is involved: `DescribeVolumes` would need an IAM change in the portal's stack and reports the provisioned size, not usage. The percentage is `df`'s (used / (used + available)), so ext4's root reserve does not let the volume read 95% when it is actually full.

**What "halted" means is decided by what writes.** The halt check joins `backtest_jobs._stop_reason()`, which the fetcher already consults before every ICICI call, so a history fetch stops at the next call. The signal backtest checks it again before each series it writes into its zip, and fails with the message (the partial zip is removed). A bot backtest's *replay* is allowed to finish on what is cached, with the halt recorded as a note like any other early fetch stop (#36): what it adds is a run row and a few-KB trail, and discarding a replay the user asked for would not free anything.

**Deletes compact, and that is why they are guarded.** A SQLite delete only marks pages free inside the file, so every delete from `backtest.sqlite3` is followed by `VACUUM`. That rewrite needs the file to itself and roughly its own size in free space. Deleting from the cache is therefore refused while a backtest runs, and a backtest cannot start while a cleanup is compacting the cache. When the volume is too full to compact, the rows are still deleted and the job says so; the freed pages show on the screen as "Deleted, not yet compacted", and the next delete retries. The job runs in a thread because compacting a 2 GB cache can outlast nginx's 60-second proxy timeout.

**What is guarded and why.** The last two sessions of futures bars are kept, because the live navbar signal warms up from them (`index_signal.warmup.cached_bars`). The log files being written now and today's bot audit file are kept, as is any bot backtest run still running. Signal backtest *rows* are never deleted, only their zips, because the 30-day bot gate reads the rows (#39). Option bars are deleted by bar date, and any fetch record overlapping the range goes too; left in place, it would tell the next replay the window was cached and the gap would never be filled.

**Sizes inside the backtest cache are estimates, marked ≈, and the option candles are never scanned.** The SQLite this image ships has no `dbstat`. Each small table is sized as row count × sampled row width (index entries included), and the option candles are sized as what the file holds beyond the rest. Their dates come from `option_fetches`, the log of fetched windows. The free pages are a line of their own, so the estimates always add up to the file, and every element together adds up to the volume's used bytes. The first version scanned `option_candles` three times per request: a `COUNT`, then `MIN`/`MAX`/distinct days, then distinct contracts. On a deployment that table is nearly the whole ~2 GB cache, with no index on `ts`. On a `t4g.small` with a 1400m container the screen outlasted nginx's 60-second timeout and showed "Request to backend failed", while the abandoned scan kept running. The same measurement on a 470 MB synthetic cache went from about 1 s of scanning to 3 ms. Concurrent requests share one measurement behind a lock, and the result is kept until the file changes.

**SPAN archives went from every revision of five dates to the latest revision per exchange of two dates** (`nsccl_baseline.SPAN_ARCHIVE_RETAIN_DATES`), about 400 MB down to about 36 MB. The margin harness can still re-read the current snapshot. For an older intraday snapshot it now gets the neighbouring revision, which `find_span_archive` already flags as inexact. The rule runs after every download and once at startup, so an upgraded volume is cleaned without waiting for the next SPAN download.

## 45. `audit_log` keeps request rows 14 days and event rows 90, and only request rows can be deleted from the Storage screen

**Decision** (2026-09-25). `audit_log` was nearly all of `users.sqlite3` (about 96% on a dev volume: 125k rows in under three months) and it had no retention, so on a 2 GiB data volume the "Accounts & settings database" became the largest thing the user could not delete. About 93% of its rows were `http_request` (one per API call, written by `middleware/request_logger.py`) and `portfolio_view` / `order_view` (one per page or chain load). They grow with how long a tab stays open, because the UI polls, not with how much the user trades. Nothing in the app reads `audit_log`: `route_audit` is not mounted, and account deletion only clears it. `services/storage/audit_retention.py` now:

* keeps request and page-view rows (`NOISE_OPS`) for 14 days, and every other row (orders, square-off rules, GTT exits, bot actions, logins) for 90 days. The trim runs at most once per process per day, started in a daemon thread by the first audit write of the day. When the trim frees at least 16 MB and the volume has room, the file is compacted too;
* shows the request and page-view rows on the Storage screen as **Request & page-view log** (group Logs, sized ≈ by sampled row width like the backtest cache's small tables), deletable by IST day. The delete is followed by `VACUUM` and `wal_checkpoint(TRUNCATE)` of `users.sqlite3`. The Accounts & settings line shows the rest of the file, and free pages get their own line, so the figures still add up.

**Why the request rows are kept at all, rather than no longer written.** The rotating log sink has method, path and status too, and it is the better debugging source. But it holds at most 7 days and about 40 MB, and on a busy day the access lines use most of that. It also drops successful polling requests on purpose (`QuietAccessPathFilter`). The `http_request` rows are the only record that a dashboard was open and polling at a given minute, which matters for "was the UI open when X happened?". Keeping them 14 days covers anything reported within a fortnight, at a fraction of the old size. Note that these rows store no status code: every one says `success`.

**Why event rows are never deletable from the screen.** Freeing space must not be able to erase the trail behind an order or square-off incident: `squareoff_rule_fire_failed` rows are what the Fired-but-stuck investigations read. Those rows are about 7% of the table, so leaving them costs little.

**Compacting a live database is safe here, and that is why no delete is refused.** Every request writes to `users.sqlite3`, unlike the backtest cache, which only a backtest writes to. `VACUUM` copies only live pages, and once the request log is trimmed that is a few MB, so other writers (5-second `sqlite3` timeout) wait well under a second. Under WAL the rewrite goes through the WAL before it is checkpointed back, so the free-space check asks for about twice the live data. When the volume is too full, the rows are still deleted and the job says so: new rows reuse the freed pages, and the next delete that has room hands them back.

## 46. The SPAN refresh runs in a child process, and the API process keeps no SPAN sheet it did not ask for

**Decision** (2026-09-26). Every SPAN refresh (the six intraday slots, the daily orchestrator, the admin button) goes through `reference_data/span_refresh_runner.refresh_all_span_baselines`. That function spawns `python -m icici_breeze_backend.workers.span_refresh`, waits for it behind one lock, reads its result from a temp file, and then drops the API process's cached SPAN sheets (`span_baseline_store.reset_local_mirror`). The in-process refresh is still there, renamed `refresh_all_span_baselines_in_process` so a new caller cannot pick it up by accident. It is what the child runs, and what the runner falls back to when Redis is in its in-memory fallback, because a child's publish would land in a store that dies with it. The publish itself now streams one sheet at a time off the primary-key index and leaves the local mirror empty; sheets load from Redis on first use, as they already did after every restart.

**What it fixed.** The 2026-09-26 log review (17–26 Sep, t4g.small, 1400m container) found that nearly every breeze_connect disconnect (`engineio.client: packet queue is empty, aborting`) landed 0–30 s into a market-hours SPAN slot. On 24 Sep at 12:45 the watchdog's re-subscribes failed for about a minute, and both scalpers closed their positions on `stale_feed` (paper then, real under live). On 25 Sep at 12:46, uvicorn alone was OOM-killed (chain-builder kept running) 38 s after an NSE ingest and the publish that followed. Measured locally on a real 130k-contract NSE file:

* ingest: ~5 s CPU on an M-series Mac (several times that on a t4g.small), and uvicorn went from 41 to 216 MB and stayed there, because CPython rarely returns a fragmented heap to the OS;
* publish: 33 → 298 MB peak, with ~214 MB of it kept for good as the local mirror (every sheet, risk arrays parsed into lists). The old mirror was still held while the new one was built, so a refresh briefly cost the API process around 480 MB;
* through the child: uvicorn 33 → 33 MB; the child peaks at ~240 MB and gives it all back when it exits. The streamed publish alone peaks at ~70 MB wherever it runs (the startup and 18:00 batch publishes still run in-process).

**Why a child process and not the chain-builder.** `exchange_margin_baseline` must have one writer, and the scheduler's docstring already required that the chain-builder never ingest. A child the API process spawns keeps a single initiator, and the lock serialises the three callers (the scheduler and the admin button could previously overlap). It also gives what a long-lived worker could not: `nice 10`; `oom_score_adj 1000`, which needs no privilege and makes the kernel take the child rather than uvicorn if the container runs short (the runner reports SIGKILL as "most likely out of memory" and the slot is simply lost); and a heap that is freed on exit instead of lingering. The admin refresh route now waits on it via `asyncio.to_thread`; it used to run the whole ingest on the event loop.
...
**Why.** From the 09:15 IST slot on 2026-09-24 every BSE refresh failed with "Could not find a published BSE SPAN archive", and the SENSEX/BANKEX baseline stayed on the 23 Sep revision. `getmaxdate/w` and `LoadData/w` return an Akamai "Access Denied" 403, from residential IPs as well as EC2, and even with a current Chrome User-Agent, Referer and Origin. That is bot protection, not an IP ban, and header tweaks do not get past it. The archives themselves still download with the app's ordinary headers. The API only ever supplied names that are fixed: the six modes map one-to-one onto suffixes, and past ingests confirm B→`00`, I–L→`01`–`04`, Z→`FINAL`, so nothing is lost by dropping it. Continuing to call it would add about eleven blocked requests per slot to an Akamai-fronted domain whose file host we still depend on.

**Details.** `source_version` keeps BSE's old mode ordinals (0–5) and labels, so ingest history and the `current_baseline_archive_name` skip check read the same across the change. A probe needs status 200 *and* a non-HTML content type: both exchanges send their misses as HTML, and a challenge page served with 200 would otherwise be taken for the archive and fail later at unzip. A holiday or weekend costs six small 404s per day walked back; weekends are not skipped, because special sessions publish.

## 48. SPAN-file margins carry an operator-set ICICI add-on from the portal, and fall back to ICICI when it is missing

**Decision** (2026-09-26). A margin priced from the exchange SPAN file is the file's own figure **plus an ICICI add-on**: a share of each short leg's notional (the SPAN file's underlying price × quantity), with a deep-OTM tier and an expiry-day extra. The seven rates are set by an operator in breeze-saas-portal (Console → Core Engine → ICICI margin add-on) and reach every deployment as the signed `margin_addon` heartbeat claim (`margin_addon.py`). Whether the SPAN file is used at all is three per-user toggles (`margin_source_prefs.py`): **Strategy Builder**, **Backtesting** and **everywhere else**. All three default to the SPAN file. Live bots always ask ICICI. The next trading day's beginning-of-day SPAN file is fetched as soon as it is published, and a 21:45 IST slot was added for NSE's.

**Why an add-on, and why it is not ELM.** Harness run `fac6f1e5` (Saturday 2026-09-26, 55 cases) repriced on every published file showed our SPAN 18–100% below ICICI's quote (median ~48%) on *every* file. As a share of short notional the gap was flat: index ATM to 5% OTM 1.97–2.31% (35 cases), index beyond 10% OTM 3.04–3.16%, stock up to 26% OTM 3.66–4.45%, a 34.5%-OTM TCS call 5.50%, long-only nil, and an iron condor the same as a naked short at the same strike. A SPAN difference would scale with risk, not notional, so this is a separate charge. ICICI's margin_calculator does not include exposure margin except on the option's own expiry day (a standing rule, recorded in CLAUDE.md). So this is modelled as an ICICI add-on and never as ELM, and the app's ELM overlays (#26, Portfolio, Strategy Builder's `provision_elm`) are untouched. The rates are the operator's, not the code's, because the stock rate in particular is not the exchange's (NSE's `FOVOLT` file puts TCS and RELIANCE 1.5σ below the 3.5% floor, yet ICICI charges ~3.9%). Measuring and changing them must not need a release.

**Why fail closed after 24 hours.** Without the add-on a SPAN-file figure understates ICICI's by 2–5% of short notional. A basket sized off that is one ICICI rejects. So a deployment that has not received the claim in 24 hours (or never has, which includes every local dev instance, since those have no portal) prices every scope from ICICI's margin_calculator. The Settings toggles then read "No", are greyed out, and show why under them. The user's stored choice is kept, and each toggle returns to it by itself when the add-on arrives. Twenty-four hours rather than the license's 2× heartbeat interval, because rates change rarely and a short portal outage should not flip every margin in the fleet. A malformed claim is dropped on its own; the license status in the same token is still honoured.

**Why the next day's file.** ICICI moves onto the next session's beginning-of-day file as soon as the exchange publishes it: NSE's `i1` at ~21:30 IST the evening before, BSE's `-00` after midnight, both stamped with the date they are *for*. The resolvers only walked back from today and the last slot was 18:00. So from then until 09:15 the next trading day, and all weekend, we priced one file behind ICICI. In `fac6f1e5` that alone moved the median error from 3.0% to 2.2%. `span_sources.next_trading_day` uses the exchange calendar to probe one beginning-of-day file ahead before walking back.

**Scopes.**
- Strategy Builder: its builds, proposals, whole-basket margin (previously forced to ICICI) and covered-shorts scan.
- Everywhere else (`margin_scope: "app"` on `/strategy-builder/margin`): Place Order, the option-chain order panel, the order confirmation dialog, Basket Order including scale-to-target, the Uncovered Shorts scan outside the Strategy Builder, and the Portfolio page's SPAN figures. A Portfolio set spanning expiries keeps ICICI's netted call, because the file-based scan does not net across expiries.
- Backtesting: lot sizing, which warns in the dialog and the run log when the SPAN file is older than the latest trading day and still runs.
- Not covered: the Dashboard's Margin used and Free margin, which are ICICI account balances rather than margin calculations.
