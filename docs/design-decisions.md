...
**Sizes inside the backtest cache are estimates, marked ≈.** The SQLite this image ships has no `dbstat`, so used pages are shared out by row count × sampled row width, index entries included. The free pages are a line of their own, so the estimates always add up to the file, and every element together adds up to the volume's used bytes.
