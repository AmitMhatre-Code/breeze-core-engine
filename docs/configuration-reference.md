...
| `AGGRESSIVE_LIMIT_ORDER_ENABLED` | `false` | ICICI has no native aggressive-limit order support yet, so this app-side workaround (order_type=market, price=0) is deactivated by default — `place_order`/`break_order`/`break_order_place_chunk` reject `aggressive_limit=true` requests and the UI toggle is hidden. Set to `true` once ICICI ships real support. |
