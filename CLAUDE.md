...
- `docs/design-decisions.md` — **why** things are shaped this way (read before "fixing" something that looks odd) — 48 numbered decisions
...
- **NIFTY/SENSEX direction signals come from one place and must be replayable.** Anything that wants bullish/bearish — navbar, the Signals page, bots — reads `app/services/index_signal/reader.py`, never an engine or its own computation, and treats `unavailable` as "no directional trade", never as `neutral`. A signal may read only what ICICI's historical API serves (one-minute OHLCV+OI futures bars) so every live reading can be backtested; nothing about a reading is stored; and a bot may use a signal only after a 30-day signal backtest — see design-decisions.md #39 and #40, and `docs/signals-streamline-plan.md`. A signal is scored at every horizon in **both** directions against a size-aware cost bar: "reliably wrong" is a finding to trade backwards, not a failed signal, and a signal trade is not closed by its own call running out.
- **A SPAN-file margin is only a stand-in for ICICI's once the portal's ICICI add-on is added** (`app/services/margin_addon.py`, design-decisions.md #48). The rates are operator-set in breeze-saas-portal and arrive in the signed heartbeat. With none received in 24h, every "use the SPAN file" toggle falls back to ICICI's margin_calculator. That is also the state of every local dev instance, which has no portal. Live bots always ask ICICI.
- **ICICI's `margin_calculator` never includes exposure margin (ELM), except when it is called on the option's own expiry day.** This is settled by the user, not a hypothesis to re-test. Any gap between our SPAN and ICICI's quoted figure on a non-expiry day is a **SPAN-side** difference and must never be explained, fitted or closed with an ELM rate. ELM in this app is our own overlay on top of ICICI's figure, and on expiry day the `same_day_expiry` guard exists because ICICI's number already contains it.
