...
from app.auth.credentials import decrypt_from_session_cookie
...
LOGIN_USER_ID_COOKIE = "login_user_id"  # short-lived; set before ICICI redirect for challenge page
...
def extract_user_context(request: Request) -> Optional[RequestContext]:
    ...
    if not token:
        return None
    # Read from app.core.config (tests set both core.config and app.core.config)
