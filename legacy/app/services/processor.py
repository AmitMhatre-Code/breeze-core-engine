...
from core.icici_client import icici_client  # tests patch app.services.processor.icici_client
...
from app.external.icici_api import fetch_customerdetails_session_token as _fetch_customerdetails_session_token
from app.external.icici_api import call_icici_api_direct as _call_icici_api_direct
...
class processor():
    ...
    def _get_full_secret_for_user(self, user_id: str, user_fragment: str = ""):
        ...
        from app.auth.context import get_full_secret_for_request
    ...
    def get_session_breeze(self, user_id):
        ...
        """
        try:
            from app.auth.context import get_broker_token_for_request, get_breeze_session_for_request, set_breeze_session_for_request
            from app.services.breeze_session_cache import get as cache_get, set as cache_set
    ...
    def get_session_token(self, user_id):
        ...
        try:
            from app.auth.context import get_broker_token_for_request
    ...
    def _maybe_evict_session(self, user_id: str, response: dict | None) -> None:
        ...
        try:
            from app.auth.context import get_broker_token_for_request
            from app.services.breeze_session_cache import evict_if_icici_auth_failure
            broker_token = get_broker_token_for_request() or ""
    ...
    def store_messages(self, user_id: str, messages: list):
        ...
        from app.repositories.message_repository import store_messages as _store
        _store(user_id, messages)

    def retrieve_messages(self, user_id: str):
        """Retrieve and flush transient messages for user."""
        from app.repositories.message_repository import retrieve_and_flush_messages as _retrieve
