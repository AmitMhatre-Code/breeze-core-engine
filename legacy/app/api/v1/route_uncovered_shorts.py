...
from app.domain.responses import UncoveredShortsDataResponse
from app.domain.trading import UncoveredShortsFormRequest
...
def get_uncovered_shorts_form_request(
    ...
    otm_call_distance: int | None = Form(None),
    otm_put_distance: int | None = Form(None),
    top: int | None = Form(None),
    action: str | None = Form(None),
) -> UncoveredShortsFormRequest:
    return UncoveredShortsFormRequest(
...
async def serve_landing(
    ...
    elm: str | None = None,
    otm_call_distance: int | None = None,
    otm_put_distance: int | None = None,
    top: int | None = None,
):
    ...
    if customer is None:
        error['location'] = "In route_uncovered_shorts.py --> serve_landing() get_customer_details returned None"
        error['contents'] = "get_customer_details() returned None for user_id = " + user_id
        breeze.store_error(error)
    elif customer['Status'] != 200:
        error['location'] = "In route_uncovered_shorts.py --> serve_landing() get_customer_details failed"
    ...
    if margin['Status'] != 200:
        error['location'] = "In route_uncovered_shorts.py --> serve_landing() get_margin_situation failed"
    ...
    if len(stock_codes) == 0:
        error['location'] = "In route_uncovered_shorts.py --> serve_landing() fetch_stock_codes failed"
        error['contents'] = "stock_codes = " + json.dumps(stock_codes)
        breeze.store_error(error)

    errors = breeze.retrieve_errors()
    if len(errors) == 0:
        if stock_code and expiry_date and limits and otm_call_distance is not None and otm_put_distance is not None and top is not None:
            parameters = {"stock_code": stock_code, "expiry_date": expiry_date, "limits": limits, "elm": elm, "otm_call_distance": otm_call_distance, "otm_put_distance": otm_put_distance, "top": top}
            options = breeze.uncovered_shorts(
                user_id,
                stock_code=stock_code,
                expiry_date=expiry_date,
                limits=limits,
        ...
        return templates.TemplateResponse(
            "uncovered_shorts.html",
            {
                "request": request,
                "is_logged_in": True,
                "login_url": None,
                "active": "uncovered_shorts",
                "customer": customer,
                "margin": margin,
                "options": options,
        ...
        return render_error_page(request, errors, active="uncovered_shorts", log_context="route_uncovered_shorts.serve_landing")

@router.post("")
async def process_post(
    request: Request,
    context: RequestContext = Depends(get_request_context_or_redirect),
    form: UncoveredShortsFormRequest = Depends(get_uncovered_shorts_form_request),
):
    user_id = context.user_id
    errors = breeze.retrieve_errors()
    ...
    if form.action == cfg.CLEAR:
        if len(errors) == 0:
            return responses.RedirectResponse(
                f"/uncovered-shorts?exchange_code={form.exchange_code or cfg.NFO}",
                status_code=status.HTTP_302_FOUND,
            )
        return render_error_page(request, errors, active="uncovered_shorts", log_context="route_uncovered_shorts.process_post CLEAR")

    if form.action == cfg.OPTIMIZE:
        return responses.RedirectResponse(
            url=f"/uncovered-shorts?exchange_code={form.exchange_code or cfg.NFO}&stock_code={form.stock_code or ''}&expiry_date={form.expiry_date or ''}&limits={form.limits or ''}&elm={form.provision_elm or ''}&otm_call_distance={form.otm_call_distance or ''}&otm_put_distance={form.otm_put_distance or ''}&top={form.top or ''}",
            status_code=status.HTTP_302_FOUND,
        )
    ...
    if form.action == cfg.SELL:
        ...
        return render_error_page(request, errors, active="uncovered_shorts", log_context="route_uncovered_shorts.process_post SELL")

    return responses.RedirectResponse("/uncovered-shorts", status_code=status.HTTP_302_FOUND)
