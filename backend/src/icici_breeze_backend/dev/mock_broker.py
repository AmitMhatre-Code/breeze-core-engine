...
class MockBreezeSdk:
    ...
    @staticmethod
    def _futures_symbol(stock_code: str, expiry_date: str, exchange_code: str = "NFO") -> str:
        """Deterministic synthetic token for a futures contract, in its exchange's room (NFO
        `4.1!`, BFO `8.1!` -- the prefixes `ws_token_index` uses for real ticks).
        ...
        prefix = "8.1" if str(exchange_code).upper() == "BFO" else "4.1"
        return f"{prefix}!{9_000_000 + digest}"
    ...
    def get_stock_token_value(
        ...
        **kwargs,
        ...
        """Resolve a subscribable token: futures only (the signal feeds' NIFTY on NFO and
        BSESEN on BFO).
        ...
        if not stock_code or not expiry_date:
            return False, False
        symbol = self._futures_symbol(str(stock_code), str(expiry_date), str(exchange_code or "NFO"))
        ...
        while stop is not None and not stop.is_set():
            for token in list(self._ws_tokens):
                if is_mock_futures_symbol(token):
                    # Seed from the real local bhavcopy spot rather than a hash: the signals
                    # key off the index's price structure, so a plausible level and a
                    # realistic lot size make the mock bars worth looking at.
                    if token not in running_state:
                        underlying = str(futures_identity(token).get("stock_name") or "NIFTY")
                        fallback = 80_000.0 if underlying == "BSESEN" else 24_000.0
                        running_state[token] = seed_running_state(
                            token, base_price=resolve_underlying_spot(underlying) or fallback
                        )
