...
def _filter_to_bhavcopy_backed(
    ctx: EngineContext,
    missing: set[tuple[int, Right]],
) -> tuple[set[tuple[int, Right]], set[tuple[int, Right]]]:
    from icici_breeze_backend.app.services.reference_data.bhavcopy_store import has_bhavcopy_quote

    fetchable: set[tuple[int, Right]] = set()
    skipped: set[tuple[int, Right]] = set()
...
def plan_targeted_fetches(ctx: EngineContext) -> set[tuple[int, Right]]:
    ...
    skipped_not_in_bhavcopy: set[tuple[int, Right]] = set()
