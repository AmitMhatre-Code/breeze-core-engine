...
def calc_bull_put_spread(ctx: EngineContext) -> StrategyResult:
    ...
    for stp_s in short_candidates:
        wing = credit_spread_wing_full(ctx, stp_s, "Put", ctx.liquid_pe_strikes, False)
        if not wing:
            continue
    ...
    if not best_wing:
        return skip(sid, name, "No bull put spread meets minimum PoP within risk limits.")
...
def prefetch_bull_put_spread(ctx: EngineContext) -> set[tuple[int, Right]]:
    ...
    for right in ("Call", "Put"):
        for strike in strikes_in_window(
            ctx.strikes, anchors, right, max_steps_for_strategy("bull_put_spread")
        ):
