"""Bull put spread strategy calculator — PoP-band search aligned with short strangle."""
...
# --- Bull put spread tuning constants (owned by this module) ---

BPS_TOP_K_SHORT_STRIKES = 10
BPS_POP_BAND_WIDTH_PCT = 2.0
BPS_SHORT_STRIKES_MAX_ATM = 12
BPS_SHORT_STRIKES_MAX = 12
BPS_SPAN_SHORTLIST_N = 10
BPS_RETURN_TOP_N = 3
MIN_BPS_ANNUALIZED_RETURN_PCT = 5.0
...
def _bps_pop_band_delta_bounds(min_pop_pct: float) -> tuple[float, float]:
    """Per-wing abs-delta range for [min_pop, min_pop + band] on single short put."""
    floor_delta = pop_to_short_delta(min_pop_pct, short_legs=1)
    ceil_delta = pop_to_short_delta(min_pop_pct + BPS_POP_BAND_WIDTH_PCT, short_legs=1)
    return ceil_delta, floor_delta


def _bps_pop_bucket(pop_pct: float, floor_pct: float) -> str:
    return pop_bucket_label(pop_pct, floor_pct, band_width=BPS_POP_BAND_WIDTH_PCT)
...
def _audit_collector(ctx: EngineContext) -> StrategyAuditCollector | None:
    if ctx.audit_collector is not None:
        ...
        c.pop_band_width = BPS_POP_BAND_WIDTH_PCT
        return c
    return None


def _bps_short_strikes_for_pop_band(
    ctx: EngineContext,
    pe_strikes: list[int],
    min_pop_pct: float,
) -> list[int]:
    ...
    cache = ctx.cache
    ceil_delta, floor_delta = _bps_pop_band_delta_bounds(min_pop_pct)
    ...
    for _, s in atm_candidates[:BPS_SHORT_STRIKES_MAX_ATM]:
        if s not in seen:
            seen.add(s)
    ...
    if not selected:
        for _, s in sorted(scored, key=lambda x: abs(x[0] - floor_delta))[:BPS_TOP_K_SHORT_STRIKES]:
            if s not in seen:
                selected.append(s)
    return selected[:BPS_SHORT_STRIKES_MAX]


def bull_put_spread_short_strikes(ctx: EngineContext) -> list[int]:
    """Return short put strikes for bull put spread optimization."""
    pe_strikes = [s for s in ctx.liquid_pe_strikes if s <= ctx.atm_strike]
    return _bps_short_strikes_for_pop_band(ctx, pe_strikes, ctx.min_pop_pct)
...
def _bps_credit_shortlist_key(cand: BullPutSpreadCandidate) -> tuple[float, float]:
    """Net credit DESC; PoP DESC for deterministic ties."""
    return (cand.net_collected, cand.pop)


def _bps_final_rank_key(
...
def enumerate_bull_put_spreads(
    ...
    *,
    ...
    """All feasible bull put spreads for a short strike, or empty if all rejected."""
    L = ctx.lot_size
...
def _build_pop_audit_summary(
    ...
    for cand in candidates:
        bucket = _bps_pop_bucket(cand.pop, min_pop_pct)
    ...
    return {
        ...
        "pop_band_target": [min_pop_pct, min_pop_pct + BPS_POP_BAND_WIDTH_PCT],
    }


def _bps_search_rationale() -> str:
    return (
...
async def _pick_top_candidates(
    ...
    *,
    ...
    span_shortlist_n: int = BPS_SPAN_SHORTLIST_N,
    return_top_n: int = BPS_RETURN_TOP_N,
) -> tuple[list[tuple[BullPutSpreadCandidate, float]], list[dict]]:
    ...
    scored: list[tuple[BullPutSpreadCandidate, float]] = []
    ...
    for credit_rank, cand in enumerate(shortlist, start=1):
        ...
        ann_return = score_bull_put_spread_candidate(
    ...
    winners = sorted(
        scored,
        key=lambda item: _bps_final_rank_key(item[0], ann_return=item[1]),
        reverse=True,
    )[:return_top_n]
...
async def calc_bull_put_spread(ctx: EngineContext) -> list[StrategyResult]:
        ...
        if ctx.audit:
            audit_calc(
                ...
                "Bull put spread candidate search",
                {
                    ...
                    "survivors": 0,
                    "pop_band_target": [ctx.min_pop_pct, ctx.min_pop_pct + BPS_POP_BAND_WIDTH_PCT],
                },
                {
                    "rejection_counts": stats.counts if stats else {},
                    "combos_tried": stats.combos_tried if stats else 0,
                },
                rationale="No bull put spread passed PoP filters.",
                strategy_id=sid,
            )
    ...
    if ctx.audit:
        audit_calc(
            ...
            rationale=_bps_search_rationale(),
            strategy_id=sid,
        )
    ...
    if not winners:
        return [skip(sid, name, "Could not resolve bull put spread finalists.")]

    if ctx.audit:
        audit_calc(
            ctx,
            "Bull put spread SPAN refinement",
            {"finalists": len(span_scores)},
            {"scores": span_scores},
            rationale=(
                f"Top {BPS_SPAN_SHORTLIST_N} by net credit shortlisted; SPAN margin fetched "
                "for finalists only; re-ranked by annualized return on SPAN, then net "
                f"credit, liquidity/spread, PoP; top {BPS_RETURN_TOP_N} returned."
            ),
            strategy_id=sid,
        )

    best_ann = winners[0][1]
    if best_ann < MIN_BPS_ANNUALIZED_RETURN_PCT:
        if stats is not None and winners:
            ...
            stats.record_near_miss(
                ...
                context=(
                    ...
                    f"{MIN_BPS_ANNUALIZED_RETURN_PCT:.1f}%."
                ),
            )
        return [
            skip(
                sid,
                name,
                f"Best bull put spread annualized return {best_ann:.1f}% below minimum "
                f"{MIN_BPS_ANNUALIZED_RETURN_PCT:.1f}%.",
            )
        ]
