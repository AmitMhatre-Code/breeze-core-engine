  ...
  },
{
  ...
  },
  {
    ...
    summary:
      "Additional Bots",
    changes: [
      "Introduced the Momentum scalper and ATM Iron Fly bots, which can be run manually or on a schedule.",
    ],
  },
  {
    version: "2.10.1",
  ...
  },
  {
    ...
    releaseKind: "minor",
    summary:
      "Bots can now trade fully unattended or ask for your Approve/Reject on Telegram first, Bot 2 picks from a shortlist of strategies instead of a fixed side, and Portfolio's PB/SL button is honest about which legs it closes.",
    changes: [
      "Bot 1 — Holdings Option Writer — now trades unattended, not just by proposal. On a firing day you set (N trading days before expiry) it sizes and places calls and puts on its own within the same per-scrip settings the manual run uses. You can now also set a target lot count per scrip for calls and puts, and a priority: a target only narrows the coverage cap, it never lets a scrip past what it can actually cover, and priority decides who gets funded first when free margin or your delivery-cash budget can't cover every scrip in full — a scrip that doesn't fit is skipped and the walk moves on, so a cheap scrip further down the list still gets funded from what's left.",
      "A holding you own can be free, pledged as collateral, or blocked for something else (a pending sale, a settlement hold) — and only the blocked portion is now excluded from call coverage. Pledged stock still counts, since it's genuinely yours and only needs unpledging before delivery; blocked stock is already committed elsewhere and isn't yours to deliver. The settings drawer's scrip table shows the split per row (free / pledged / blocked) so a lot cap is explainable at a glance instead of just a single total.",
      "Bot 2 — Expiry-Day Index Writer — now picks from a shortlist instead of a fixed side. Per index, choose any combination of naked call, naked put, and short strangle; with more than one enabled, the bot trades whichever earns the most premium per rupee of margin at fire time — not the most premium outright, since a strangle collects more than either leg alone every time and ranking on raw premium would always pick it regardless of whether the extra margin it ties up is worth it. Both legs of a strangle are priced through the broker's margin calculator together, so the comparison reflects the exchange's real netting benefit.",
      "Bot 2's profit target is now set as a share of the premium collected rather than an absolute option price — \"book at 60% of premium\" instead of a rupee figure — matching how the loss limit already works as a multiple of premium, so the target means the same thing regardless of what the option happened to cost. Set it to 100% and nothing is armed on the profit side at all — the position is deliberately left to run to expiry worthless, with only the stop-loss live. On a strangle, both legs have to reach their target before either is bought back, so booking profit can't leave one side of the pair naked.",
      "Bots now run in a priority order you set per bot, and margin one bot commits is subtracted from what the next one sees — so if both are due to fire the same morning, the first one through doesn't leave the second sizing against margin that's already spoken for.",
      "New: ask on Telegram before trading. Each bot can be set to \"Trade on its own\" (the existing behaviour) or \"Ask me on Telegram\" — it still sizes the trade itself on schedule, but instead of placing it, sends you the priced legs with Approve / Reject buttons and places nothing until you tap Approve. Silence never trades: if you don't respond, it re-sends at fresh prices every few minutes until your cutoff time, then skips the day. A rejection ends the day outright, and an approval whose price has since moved is refused with a fresh proposal sent in its place, rather than filling you at a number you didn't see. A bot set to ask with no Telegram account linked can't ask, so it can't trade — and the settings drawer warns about this up front rather than letting it fail as a silent no-trade day.",
      "The Bots page now shows two square cards, one per bot, with what it'll do next in plain terms (\"Fires 2 trading days before expiry, from 09:00\") and what it last did (\"28 Aug · 4 placed\") always visible. A gear icon opens a settings drawer with tabs for scrips/indices, schedule, and limits; \"Start a run\" opens a full-screen sheet that prices what the bot would do right now, for you to edit and execute by hand whenever you don't want to wait for its schedule.",
      "Portfolio's Profit Booking / Stop Loss button on a strategy group is now disabled until every open leg in the group is ticked, with a hint explaining why (\"Select all 3 legs to apply\") — the rule the engine arms always closes the whole group, so a button sitting next to a partial selection was misleading about its own scope. A new \"+ Set Profit Booking / Stop Loss\" line under a collapsed group's summary expands it and ticks every leg for you in one action, instead of asking you to expand and select by hand first. The \"Hedge\" suggestions button has been removed from Portfolio's position groups.",
    ],
  },
  {
    version: "2.9.0",
    ...
    releaseKind: "minor",
    summary:
      "New Bots section: a holdings-backed option writer that proposes trades for you to approve, and an expiry-day index writer that trades unattended within caps you set.",
    changes: [
      "New Bots section in the main navigation, holding two bots and a run log. Each bot has its own settings panel and an on/off switch, and every run a bot makes — a scan, a proposal, an order, or a decision not to trade — is recorded with the reason for it. A day on which a bot did nothing now says why it did nothing (\"no broker session by 12:00\", \"one lot exceeded your margin cap\", \"not an expiry day\"), so a quiet bot can be told apart from a broken one without reading server logs.",
      "Bot 1 — Holdings Option Writer. Scans your NSE holdings for stocks that have monthly option contracts and proposes a set of options to write against them; nothing is placed until you review the list and approve it. Calls are the genuinely covered side and are capped by the stock you actually hold, with lots you've already written deducted — netted across every open expiry, so it can't oversell your coverage. Puts are opt-in per scrip and aren't covered by holdings at all — a put assigned against you buys shares, which needs cash — so they're capped by a single delivery-cash budget you set, with each candidate showing its assignment cost and a running total for you to allocate by hand. Strikes come from a safety % you set per scrip, always snapped away from the current price rather than towards it, and margin is shown per row and as a basket total, netted against the positions you already hold.",
      "Bot 1 shows what it can't be certain about, and refuses to trade on it. Premium is quoted at the bid — stock-option spreads are too wide for a last-traded price to mean much — but outside market hours there is no bid at all, so a proposal built on a Sunday prices off the last trade and marks those legs \"indicative\" in amber. Approval re-prices every selected leg against the live book and will not place one that is still indicative; it also aborts if a bid has moved more than 10% since the scan, so you re-approve against real prices instead of stale ones. Shares you have pledged are called out per scrip too, since they must be unpledged — a settlement cycle — before they can be delivered against an assigned call.",
      "Bot 2 — Expiry-Day Index Writer. On NIFTY and SENSEX expiry days it writes a short option and manages the exit on its own, within caps you configure per index: which side (call or put), how far from spot, and what share of your free margin it may consume (30% by default). Expiry days are read from the exchange's own contract list rather than assumed from the weekday, since those have moved before. Size is estimated from the SPAN baseline and then verified against ICICI's margin calculator before anything is sent; if even one lot exceeds your cap it skips the day and says so, rather than trading a partly funded position.",
      "Bot 2 arms its own exit the moment it fills, with two independent triggers: a loss limit set as a multiple of the premium collected (1× by default, so a stop at 100% loss), and a profit target set as an option price — exit when the option trades at or below, say, ₹0.10. The price target is deliberately a price and not a rupee P&L: a rupee target is measured against the broker's average price, which can drift from the price the bot actually filled at, and would quietly move the exit away from the level you asked for. If a position opens but its stop can't be armed, the run is reported as failed and says so explicitly — that is the worst state this bot can leave behind and it never reads as a clean trade.",
      "Because the ICICI session lapses overnight, a bot due to trade on an expiry morning nags you on Telegram from 08:00, every 15 minutes, until you log in — and stops the moment you do. It enters at your scheduled time (09:30 by default) if a session exists by then, or as soon as one appears after that, up to a 12:00 cutoff. No session by 12:00 and the day is skipped with that recorded as the reason.",
      "Bots respect the same licence rules as the rest of the app: a deployment in read-only mode can't enable one, and says so on the card rather than accepting the switch and then silently skipping every run. A run interrupted by a restart or a version upgrade is closed out as \"interrupted\" on the next start — pointing you at the Order Book, because an interrupted run may already have placed orders before it stopped.",
    ],
  },
  {
    version: "2.8.0",
